...
void Lista::Dodaj(int wartosc){
    ...
    if(head == NULL) {
        ...
        tail->nastepny = el; //ustawienie wskaźnika nastepny dotychczasowego ostatniego elementu na adres nowego elementu
        tail = el;// aktualizacja wskaźnika doostatniego elementu
    }
}
...
bool Lista::Usun(){
    if(head != NULL) { // lista zawiera elementy
        if(head == tail) { // usuwamy ostatni element
            ...
            ELEMENT *el = head;
            while (el->nastepny != tail) { // szukam przedostatniego elemenytu
                el = el->nastepny;
            }
